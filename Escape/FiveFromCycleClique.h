...
CycleBased initfourCycleBasedCounter(CGraph *gout, CGraph *gin, TriangleInfo *outinfo, TriangleInfo *ininfo)
{
   ...
   EdgeIdx *outout_count = new VertexIdx[gout->nEdges+1]; // stores the counts of outout wedges from a vertex
   EdgeIdx *inout_count = new VertexIdx[gout->nEdges+1];  // stores the counts of inout wedges from a vertex
   VertexIdx i,j,k;
...
if(DEBUG)
{
               printf("Consider %ld <- %ld -> %ld\n",i,j,k);
               printf("Info: outout_count[%ld] = %ld, inout_count[%ld] = %ld, total_tri = %ld\n",k,outout_count[k],k,inout_count[k],total_tri);
               printf("Degs: degi = %ld, degj = %ld, degk = %ld\n",degi,degj,degk);
               printf("Adding %ld to type1hatted, %ld to type3hatted\n",(outout_count[k]-1)*total_tri,inout_count[k]*total_tri);
               printf("Adding %lf to type1tailed, %ld to type3tailed\n\n",((double)(degk-2)/2 + (double)(degi-2)/2 + (double)(degj-2))*(outout_count[k]-1),(degk-2 + degi-2 + degj-2)*inout_count[k]);
}
            }
/*               to_add = (outout_count[k] - 1)*(degj - 2)/2 + inout_count[k]*(degj-2); // (i,j,k) form a type1 4-cycle with every other outout wedge. An edge incident to j gives the tail. This pattern is touched twice, so we divide by 2
               ...
               {
                    printf("Incrementing tailed %ld in 1st\n",to_add);
                    printf("i = %ld, j = %ld, k = %ld\n",i,j,k);
                    printf("outout_count[k] = %ld, inout_count[k] = %ld, degj = %ld\n\n",outout_count[k], inout_count[k], degj);
               }
               if (always_print || to_add_hatted > 0)
               {
                    printf("Incrementing hatted %ld in 1st\n",to_add_hatted);
                    printf("i = %ld, j = %ld, k = %ld\n",i,j,k);
                    printf("outout_count[k] = %ld, inout_count[k] = %ld, total_tri = %ld\n\n",outout_count[k], inout_count[k], total_tri);
               }*/
...
//    printf("type1hatted = %ld, type2hatted = %ld, type3hatted = %ld\n",(long)type1hatted, (long)type2hatted, (long)type3hatted);
//    printf("type1tailed = %ld, type2tailed = %ld, type3tailed = %ld\n",(long)type1tailed, (long)type2tailed, (long)type3tailed);
...
//                         if (DEBUG)
//                             printf("%ld %ld %ld: checking %ld %ld\n",i,j,k,ell,oh);
